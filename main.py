...
def fetch_market_data(ticker):
    raw = yf.Ticker(ticker, session=session).info
    return {
        ...
        'de': (raw.get('debtToEquity', 0.0) or 0.0),
...
with st.sidebar:
    ...
    # CAMBIO: Ticker inicial a V para resetear el foco de la API
    ...
    if st.button("🚀 Cargar Métricas de Mercado"):
        try:
            # Llamada a la función con Caché y Session
...
if d:
    ...
    # Lógica de Factor ORCA (Manteniendo tu MAX original)
